...
import { EventDto, NewRegistrationDto, ProductDto, RegistrationType, UserDto } from "@losol/eventuras"
...
const AddUserCard:React.FC<AddUserCardProps>=({user,event,onRemove})=>{
  ...
  return <form className="bg-slate-100 rounded-xl p-8 dark:bg-slate-700 m-4" onSubmit={handleSubmit(onSubmitForm)}>
    ...
    <Button variant="light" onClick={()=>{onRemove(user)}}>Clear</Button>
  </form>
}
...
const AddUserToEvent: React.FC<AddUserToEventProps> = ({event}) => {
  ...
  />
  {usersToAdd.map(user=><AddUserCard user={user} event={event} key={user.id} onRemove={(u=>{
